...
public class AuthenticationService {
    ...
    public User signup(RegisterUserDto input) {
        ...
        user.setVerificationExiresAt(LocalDateTime.now().plusMinutes(10));
    ...
    public User authenticate(LoginUserDto input)
    {
        ...
        if(!user.isEnabled())
        {
            throw new RuntimeException("Please verify your account to be able to Sign In");
        }
    ...
    public void verifyUser(VerifyUserDto input)
    {
        ...
        if(optionalUser.isPresent())
        {
            ...
            if(user.getVerificationExiresAt().isBefore(LocalDateTime.now()))
            {
            ...
            if(user.getVerificationCode().equals(input.getVerificationCode()))
            {
                ...
                user.setVerificationExiresAt(null);
    ...
    public void resendVerificationCode(String email)
    {
        ...
        if(optionalUser.isPresent())
        {
            ...
            user.setVerificationCode(generateVerificationCode());
            user.setVerificationExiresAt(LocalDateTime.now().plusMinutes(20));
            sendVerificationEmail(user);
            userRepository.save(user);
        } else {
            throw new RuntimeException("User not found");
